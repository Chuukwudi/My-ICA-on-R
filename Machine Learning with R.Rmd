...
ggplot(dataset,
 ...
 geom_point(aes(colour = AIDangerous), show.legend = FALSE) + 
 labs(x = "AI Dangerous", 
      y = "Salaries",
...
ggplot(PlotIDE, aes(x = reorder(Type, pct),
  ...
  geom_bar(stat = "identity", 
           ...
           color = "cornsilk3") + 
  scale_y_continuous(labels = percent) + 
  labs(x = "Version Control Type", y = "Percent", title  = "Participants by VersionControl")+
  coord_flip()
```
...
To prepare our table for machine learning, I would like to examine my choices. My Target variable (ConvertedSalaray) which I want to predict is a continuous variable while the rest of my variables are categorical. It is best practice to convert categorical variables to continous variables if your target variable is continous in nature. This is because the Machine Learning algorithms that are used to predict continous variables perform better with continous inputs.

to convert my data, I am going to categorise them into 2. 
The Ordinal variables and
The Norminal variables.


My Ordinal variables are those variable with ordered factor. They are CompanySize, YearsCoding, YearsCodingProf, NumberMonitors. I am going to encode them using a technique called "Integer Encoding". This involve equating the variables themselves to the number of their factor level.

The Norminal variables are my variables which do not have order. All my variables are norminal variables except CompanySize, YearsCoding, YearsCodingProf, NumberMonitors and lastly, ConvertedSalary which is the only truly continuous variable I have in my data. The method of encoding the nominal variable will be different from the way I will encode my ordinal data. With my norminal data, I will use one hot encoding to encode them. I have addressed this technique earlier.

Encoding Employment

```{r}
summary(dataset$Employment)
```
This shows that Employment column only contains 2 levels. Encoding this will be very easy. We simply make "Employed full-time" = 1 and "Employed part-time" = 0.

```{r}
rawdataset <- dataset 
# I just want to save my dataset to a variable incase I need to make some 
# reference in the future.

dataset$Employment <- ifelse(dataset$Employment == "Employed full-time", 1, 0)
summary(dataset$Employment)

#View(do.one.hot(rawdataset$VersionControl, separator = ";"))
```

Now, let us encode the columns that contain ordinal variables.

```{r}
dataset$CompanySize <- as.numeric(factor(dataset$CompanySize, levels = unique(dataset$CompanySize), exclude = NULL))
dataset$YearsCoding <- as.numeric(factor(dataset$YearsCoding, levels = unique(dataset$YearsCoding), exclude = NULL))
dataset$YearsCodingProf <- as.numeric(factor(dataset$YearsCodingProf, levels = unique(dataset$YearsCodingProf), exclude = NULL))
dataset$NumberMonitors <- as.numeric(factor(dataset$NumberMonitors, levels = unique(dataset$NumberMonitors), exclude = NULL))
```

Now, let us encode the column that contains the Norminal data. We will begin by creating a function in order to eliminate repetition.
```{r}
One.hot <- function(dataset, column){
 # This funtion takes in two variables, a dataset and a column and returns a
 # a dataset which has that column one hot encoded.
 
 string <- paste("~", toString(column))
 # We use our column to create a string which will depend on the input.
 
 dum <- dummyVars(string, data = dataset)
 # we dummify our dataset.
 
 trsf <- data.frame(predict(dum, newdata = dataset))
 # we get the one-hot encoded dataframe partaining to the column
 
 new.data <- cbind(dataset, trsf)
 # we bind the new dataframe to our original dataframe
 
 new.data <- select(new.data, -c(toString(column)))
 #we remove the dataframe we just converted.
 return(new.data)
}
```

Now that we are done creating our function, let us go ahead and apply one hot encoding and get what we want.
```{r}
dataset <- One.hot(dataset = dataset, column = "Country")
dataset <- One.hot(dataset = dataset, column = "AIDangerous")
dataset <- One.hot(dataset = dataset, column = "AIInteresting")
dataset <- One.hot(dataset = dataset, column = "AIResponsible")
dataset <- One.hot(dataset = dataset, column = "AIFuture")
```

Now, the non numeric columns we have left are those columns that are formated in a specific way such as the "DevType" column. Let us look at the first 2 rows.
```{r}
print(head(dataset$DevType, 2))
```
I will use the function "do.one.hot" which I initially created during my visualisation process to encode my datset.
Here is a function to encode, combine, delete column and return a one hot encoded dataset.
```{r}
...
dataset <- One.hotter(dataset = dataset, column = "OperatingSystem", dataset.with.column = dataset$OperatingSystem)
...
Now we have 364 columns of data. all numeric
...
```{r}
rmPCA.worthy <- subset(PCA.worthy, select = -c(ConvertedSalary))
rmPCA.worthy <- unique(rmPCA.worthy)# remove double entries.
rme <- scale(rmPCA.worthy)
#View(rmPCA.worthy)
#feature selection
set.seed(111)
boruta <- Boruta(ConvertedSalary ~., data = PCA.worthy, doTrace = 2)

# SELECT IMPORTANT VARIABLES ONLY.
# SECT FIRST FIVE COUNTRIES, ELIMINATE RUBBISH VARIABLES.
```
